...
class PdfExportService {
  static Future<File> generateRecipePDF(FoodInfoById recipe) async {
    ...
    if (recipe.images.isNotEmpty) {
      try {
        final response =
            await http.get(Uri.parse(recipe.images.first.imageUrl));
              ...
              children: [
                pw.Column(
                  children: [
                    ...
                    pw.Text('${recipe.servingSize}', style: bodyStyle),
                  ],
                ),
                pw.Column(
                  children: [
                    pw.Text('Cook Time',
                        style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
                    pw.Text('${recipe.totalCookTime ?? 'N/A'}',
                        style: bodyStyle),
                  ],
                ),
                pw.Column(
                  children: [
                    pw.Text('Difficulty',
                        style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
                    pw.Text('${recipe.difficulty ?? 'N/A'}', style: bodyStyle),
                  ],
                ),
                pw.Column(
                  children: [
                    pw.Text('Category',
                        style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
                    pw.Text('${recipe.category ?? 'N/A'}', style: bodyStyle),
                  ],
                ),
              ],
            ),
                      ...
                      child: pw.Row(
                        ...
                        children: [
                          pw.Container(
                            width: 5,
                            height: 5,
                            margin: pw.EdgeInsets.only(top: 4, right: 5),
                            decoration: pw.BoxDecoration(
                              color: PdfColors.black,
                          ...
                          pw.Expanded(
                            child: pw.Text("${i.quantity} ${i.name}",
                                style: bodyStyle),
                          ),
                        ],
                      ),
          ...
          if (recipe.nutritionalContent.isNotEmpty) ...[
            ...
            pw.Container(
              ...
              child: pw.Wrap(
                spacing: 20,
                runSpacing: 8,
                children: recipe.nutritionalContent
                    .map((e) => pw.Container(
                          width: 150,
                          child: pw.Row(
                            crossAxisAlignment: pw.CrossAxisAlignment.start,
